...
'    This script based on filterByCoordinate.py in radia release 1.1.3
'
...
def get_snp_data(anInputStream, anIsDebug):
    '''
    ' Tries to guess if data comes from VCF or bed
    ' The filter step only allows a single base as query, therefore
    ' the SNP vcf gets parsed without trying to calculate the full coordinate range
...
def filter_events(aTCGAId, aChrom, aSnpFilename, aVCFFilename, anOutputFilename, aFilterName, aFilterField, anIncludeOverlapInfo, anIncludeFilterName, anIncludeIdName, aFilterHeaderLine, anIsDebug):
    '''
    ' The function reads from a snp .bed or .vcf file and a .vcf file line by line and looks for variants that should be
    ' filtered out.  The .bed file specifies coordinates for areas where variants should either be included (default)
    ' or excluded. 
    '
    ' aTCGAId: The TCGA Id for this sample
    ' aChrom: The chromosome being filtered
    ' aSnpFilename: A .vcf file or .bed file with at least 4 columns specifying the chrom, start, and stop coordinates and an id
    ...
    i_filterFileHandler = get_read_fileHandler(aSnpFilename)
    ...
    (i_dbSnpDict) = get_snp_data(i_filterFileHandler, anIsDebug)
